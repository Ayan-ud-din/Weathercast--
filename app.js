


 const apikey = "9e43c36c41314c899ce74623250612";

const cityNameInp = document.getElementById("cityInp")

const searchBtn = document.getElementById("searchBtn")

const main = document.getElementById("main")




searchBtn.addEventListener("click" ,()=>{
getWeather(cityNameInp.value)
})



const getWeather = async (cityName) => {
  console.log("city name ==>", cityName);


  try{
    // main.innerHTML = '';

    main.innerHTML = `<div class="d-flex align-items-center">
  <strong>Loading...</strong>
  <div class="spinner-border ms-auto" role="status" aria-hidden="true"></div>
</div>`

   const res = await fetch(
      `https://api.weatherapi.com/v1/current.json?key=${apikey}&q=${cityName}&aqi=no`
   ) ;
   const data = await res.json();
   console.log(data);
    if (data.error) {
      main.innerHTML = `<p style="color:red;">City not found! Try again.</p>`;
      return;
    }

   const tempC = data.current.temp_c
                      
   const tempF = data.current.temp_f
   const citiesName = data.location.name
   const feelsLike = data.current.feelslike_c
   const condition = data.current.condition.text
   const windSpeed =  data.current.wind_kph
   const humidity = data.current.humidity


main.innerHTML= `
      <hr>


<div class="weatherImg"><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnxR0aLchD66Yf5hL9Lx_hbAm1NzCl3AT0NsEIDFiUwQ&s" alt=""></div>

<div class="cityName"><p style="font-size: 35px; ">${citiesName}</p><p>${condition}</p> </div>

<div class="temp" ><p style="font-size: 85px;font-weight:light;   " >${tempC}°C </p>
<sub style="font-size: 25px;  ; " >   ${tempF}°F </sub></div>

<hr>

<div class="details" >
   <p style="font-size:15px;   " >Feels like : ${feelsLike}| </p>
   <p style="font-size: 15px;   " > Wind speed : ${windSpeed} | </p>
   <p style="font-size: 15px;   " > Humidity : ${humidity} </p>
</div>


<div class="footer" ><p style="font-size: small;color: rgb(96, 103, 109);">Developed by © Ayan uddin | All rights reserved ©</p></div>


<div class="socialHandles"><a href="https://github.com/Ayan-ud-din"><img src="data:image/png;base64," alt="" width="50px" srcset=""></a></div>

`


  }
  catch(error){
console.log(error);

  }

}
